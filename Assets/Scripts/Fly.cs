...
public class Fly : MonoBehaviour
{
    ...
    void FixedUpdateEvents()
    {
        ...
        if (_frog != null)
        {
            ...
            //Check if its outside the range of the bubbles
            bool outsideBubbles = true;
            foreach (GameObject bubble in GameObject.FindGameObjectsWithTag("Bubble"))
            {
                if ((transform.position - bubble.transform.position).magnitude < StopFleeingRange)
                {
                    outsideBubbles = false;
                    break;
                }
            }

            // Check if we no longer need to be scared
            if ((transform.position - _frog.transform.position).magnitude > StopFleeingRange && outsideBubbles)
            {
